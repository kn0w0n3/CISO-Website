<?php
	if (isset($_POST['key'])) {
		...
		$conn = new mysqli('localhost', 'cisoguum_cisoagkc_ciso_members', '1nf0$3cC1$0W3b', 'cisoguum_cisoagkc_ciso_members');
		
		if ($_POST['key'] == 'getRowData') {
			$rowID = $conn->real_escape_string($_POST['rowID']);
			$sql = $conn->query("SELECT id, first_name, last_name, student_id, email, year, cybersquad, signup_date, dues_paid, date_paid, email_confirmed FROM other_members WHERE id='$rowID'");
		...
		if ($_POST['key'] == 'getExistingData') {
			...
			$sql = $conn->query("SELECT id, first_name, last_name, student_id, email, year, cybersquad, signup_date, dues_paid, date_paid, email_confirmed FROM other_members LIMIT $start, $limit");
			if ($sql->num_rows > 0) {
				$response = "";
		...
		if ($_POST['key'] == 'deleteRow') {
			$conn->query("DELETE FROM other_members WHERE id='$rowID'");
		...
		if ($_POST['key'] == 'updateRow') {
			$conn->query("UPDATE other_members SET first_name='$fname', last_name='$lname', student_id='$sid', email='$email', year='$year', cybersquad='$cybersquad', dues_paid='$dues_paid', date_paid='$date_paid', email_confirmed='$email_confirmed'  WHERE id='$rowID'");
			exit('success');
		}
			
		if ($_POST['key'] == 'addNew') {
			$sql = $conn->query("SELECT email FROM other_members WHERE email = '$email'");
			if ($sql->num_rows > 0)
				exit("Email Already Exists!");
			else {
				$conn->query("INSERT INTO other_members (first_name, last_name, student_id, email, year, cybersquad, signup_date, dues_paid, date_paid, email_confirmed) 
